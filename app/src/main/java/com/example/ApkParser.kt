...
object ApkParser {
    ...
    // Lists contents of selected APK from File (Memory efficient)
    fun listApkEntries(file: File): List<ApkEntry> {
    ...
    // Lists contents of selected APK from ByteArray (Compatibility)
    ...
    // Extract exact file from APK file (Memory efficient)
    fun extractEntryBytes(file: File, entryName: String): ByteArray? {
        try {
            val fis = java.io.FileInputStream(file)
            val zis = ZipInputStream(fis)
            var entry = zis.nextEntry
    ...
    // Extract exact file from APK bytes (Compatibility)
